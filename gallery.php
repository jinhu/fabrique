...
$db_found = mysql_select_db("$dbDatabase", $db) or die ("Couldn't select the database.");
	echo "";
if ($result=mysql_query("select * from ".$dbTable."", $db)) {
	...
	while($row = mysql_fetch_array($result))
	  {
		...
		echo "<div class='gallery' id='".$row['id']."' style='float:left;width:200px;height:100px;padding:10px;margin:10px;border:5px solid #aaa;'>";

		echo "<h2 style='margin-top:0;'>".$rowtitle."</h2>";

		echo $desc;

		echo "</div>";
		echo "<script type='text/javascript'>";
		echo "$('#".$row['id']."').click(function() { $(this).css('background', '#eee'); window.location = '?f=".$row['id']."'; })";
		echo "</script>";
	  }
}
?>